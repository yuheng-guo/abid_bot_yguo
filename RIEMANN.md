...
The ~3.5 TB of sol_32 output is too big to copy into the abid_bot tree, so it
stays where it is and is symlinked in -- the same convention the other abid_bot
installs on riemann use. `params` points at it:

    h5src="/data/yuhengguo/bhdisk_sol_32/hdf5/h5_extracted"

`bin/link_h5data.sh` does three things:

1. moves the `sol32_may_hdf5/` folders up into `$h5src` (they fill the
   26_05_05 - 26_05_25 gap),
