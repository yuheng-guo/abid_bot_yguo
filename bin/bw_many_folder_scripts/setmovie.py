...
# distutils left the stdlib in Python 3.12 and resolves there only when
# setuptools happens to shim it. This is all we used from it.
def strtobool(v):
    v = str(v).strip().lower()
...
# bhcen1.txt is generated by setup_bh.sh (center_lister.py) from your horizon
# data. setup.sh runs setup_bh.sh only when h5data/horizon/ exists -- say so
# plainly rather than raising a bare FileNotFoundError from three layers down.
