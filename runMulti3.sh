...
# --- set 3: the GW-composite disk frames -------------------------------------
# These feed Blender_Gravitational_Waves, which billboards each frame over the
# wave mesh. It needs an alpha-0 background (an opaque one would hide the waves)
# and no t/M caption (Blender draws its own time label).
export NO_TIME_LABEL=1
export TRANSPARENT_BG=1
# riemann has ~111 idle cores and each VisIt serial engine takes ~1; 40 keeps a
# wide margin for the other users on the box.
maxParallel=40
...
firstFolder=1
lastFolder=0
framesPerRun=115  # 4591 frames / 40 = 40 balanced jobs, one wave at maxParallel=40



jobName=bhdisk_sol32_diskgw_meshmatch
h5dir=$root/h5data
